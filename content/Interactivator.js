...
Wistia.plugin("interactivator", function (video, options) {
	...
	if (video.hashedId() != WistiaId) {
		...
		video.addToPlaylist(WistiaId,
			{
				transition: "none", autoPlay: false, fakeFullscreen: iOS,
				plugin: {
					"interactivator": {
	...
	let courseDataFile = "https://videos.getsmarter.com/Interactive+Video+Content/courseData/" + courseCode + ".js"
	let UPDataFile = "https://videos.getsmarter.com/Interactive+Video+Content/" + up + "/" + up + ".js"
	let UPcss = "https://videos.getsmarter.com/Interactive+Video+Content/" + up + "/" + up + ".css"
	
	let cssFile = "https://videos.getsmarter.com/Interactive+Video+Content/generic.css";
	let standard_generators = "https://videos.getsmarter.com/Interactive+Video+Content/standard_generators.js"
	...
	addScript("https://getsmartervideos.s3.eu-west-1.amazonaws.com/Interactive+Video+Content/standard+generators/lottie.js", "lottie");
	...
	function addFrame() {
		...
		elem.innerHTML = '<span  id="frame" style="background-color: black; pointer-events: auto;">';
	...
	function timeAnimation(animation, start) {
		...
		if (animation.isLoaded) {
			...
			video.bind("timechange", function (timeNow) {
				...
				if ((video.state() === "paused") && animation.isLoaded && (lottieTime > 0)) {
			...
			setTimeout(function () { timeAnimation(animation, start) }, 100)
		}
	}
	...
	function playbuzz(enterTime, endTime, link, scale, yAxis) {
		...
		js.innerHTML = "(function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(d.getElementById(id))return;js=d.createElement(s);js.id=id;js.src='https://embed.ex.co/sdk.js';fjs.parentNode.insertBefore(js,fjs);}(document,'script','exco-sdk'));console.log('added!');";
		frame.appendChild(js);

				if (scale == "") { scale = 1 }
				var id = link.split('/')[link.split('/').length - 1]
				if (id == '') { id = link.split('/')[link.split('/').length - 2] }
			...
			if (withinInterval) {
				...
				event.innerHTML = `
						<div id="background">
							...
							<div id="textContainer">
								...
								<textarea autofocus id="studentInput">`+ prompt + `</textarea>
							</div>
							</div>
						</div>
					`;
				...
				studentInput.addEventListener("input", function () {
					if (studentInput.value.length > (prompt.length)) {
						document.getElementById("flip-card-container").style.pointerEvents = "auto";
	...
	function lowerThird(enterTime, endTime, speaker, Left_or_Right, ltAnim) {
		...
		if (ltAnim.isLoaded && (document.getElementById('Name').getElementsByTagName('tspan').length > 0)) {
			// try {
			...
			for (let i = 0; i < textElements.length; i++) {
				let fontSize = window.getComputedStyle(textElements[i], null).getPropertyValue('font-size')
			...
			if (longestTextWidth > 150) {
				...
				let holdingShape = document.getElementById('holdingShape').firstChild.firstChild
			...
			setTimeout(function () { lowerThird(enterTime, endTime, speaker, Left_or_Right, ltAnim) }, 100)
		}
	};
	...
	function backgroundAndTrans(start, end) {
		...
		if (!transAnim.isLoaded) {
			setTimeout(function () { backgroundAndTrans(start, end) }, 100)
		} else {
	...
	function heading(enterTime, endTime, text, offset, Left_or_Right) {
		...
		video.bind("betweentimes", enterTime, endTime, function (withinInterval) {
			if (withinInterval) {
				...
				document.getElementsByTagName("video")[0].style.left = 25 + parseInt(offset) + rightsideOffset + "%";
				if (logo) {
					logo.style.display = "none";
				}
				for (var i = 0; i < document.getElementsByClassName("generator_background").length; i++) {
					if (document.getElementsByClassName("generator_background")[i] != generator_background) {
						document.getElementsByClassName("generator_background")[i].style.display = "none"
					}
				}
			} else {
		...
		if (text != "") {
			...
			heading1.innerHTML = '<div class="headingtext">' + text[0].toUpperCase() + '</div>'
			var heading2 = document.createElement('div')
			if (text.length > 1) {
				heading2.innerHTML = '<div class="headingtext">' + text[1].toUpperCase() + '</div>'
			} else {
			...
			function addHeading(heading, enterTime, endTime, className) {
				video.bind("betweentimes", enterTime, endTime, function (withinInterval) {
					if (withinInterval) {
						if (((video.time() > enterTime) && (video.time() < enterTime + 0.8) && (video.state() == "playing")) || ((video.time() == enterTime) && (video.state() == "paused"))) {
							heading.className = className + " transition"
						} else {
							heading.className = className
						}
						if (Array.from(generator_background.children).indexOf(heading) == -1) {
	 ...
	 function Video_Interactivity_Timestamp(enterTime, endTime) {
		...
		let timer = setInterval(() => {
			...
			console.log(parseInt(enterTime));
	...
	function chapter(enterTime, text) {
		...
		if (startChap < 0) { startChap = 0 }
		if (startChap == 0) { startPlay = false }
		...
		video.bind("betweentimes", startChap, endTime, function (withinInterval) {
			if (withinInterval) {
				makeiOSSafe()
				generator_background = document.getElementById("chapter_background/" + enterTime)
				generator_background.style.display = "flex"
		...
		video.bind("betweentimes", parseFloat(enterTime), endTime, function (withinInterval) {
	...
	function bullet(enterTime, endTime, text) {
		...
		var generator_backgrounds = document.getElementsByClassName("generator_background")
		if (generator_backgrounds.length > 0) {
			...
			for (var i = 0; i < generator_backgrounds.length; i++) {
				...
				if ((parseFloat(scope[0]) <= enterTime) && (parseFloat(scope[1]) >= enterTime)) {
			...
			if (generator_background) {
				...
				bullet.innerHTML = `<div class="bulletpoint">&bull;</div><div class="bullettext">` + text + `</div>`
				bullet.firstChild.onclick = function () { video.time(enterTime) }
				bullet.id = 'bullet' + enterTime + text
				video.bind("betweentimes", enterTime, endTime, function (withinInterval) {
					if (withinInterval) {
						if (((video.time() > enterTime) && (video.time() < enterTime + 0.4) && (video.state() == "playing")) || ((video.time() == enterTime) && (video.state() == "paused"))) {
	...
	function execute(chapters, functions) {
		...
		try {
			...
			if (lwrThirdData && transtition1Data && lottie && bodymovin && graphicsContainer) { proceed = true }

		} catch (err) {
			setTimeout(function () { execute(chapters, functions) }, 100)
		}
		if (proceed) {
			generalLottie()
